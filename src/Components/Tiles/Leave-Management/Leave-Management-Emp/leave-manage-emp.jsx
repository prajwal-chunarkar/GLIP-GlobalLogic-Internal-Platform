...
const LeaveManageEmp = () => {
  ...
  var count = 0;          // for rows pagination
  ...
  const { leaveType, startDate, endDate, duration, isHalfDay, halfDayDate, reason, actionBy, rejectReason, status } = leaveRequest;
  ...
  const fetchdata = async () => {
    await axios.get(`http://localhost:3003/users/${id}`)
      .then((res) => {
        ...
        axios.get(`http://localhost:3003/leave-requests`)
          .then((resp) => {
            resp.data.forEach((obj) => {
              if (obj.empID === res.data.empID) {
                setTotalLeaves(totalLeaves => [...totalLeaves, obj]);
                count++;                 //for pagination rows
              }
            })
            setRows(count)
          });
        axios.get(`http://localhost:3003/leaves-remain`)
          .then((respo) => {
            respo.data.forEach((object) => {
              if (object.empID === res.data.empID) {
                setLeavesRemain(object);
                console.log(object)
              }
            })
          });
  ...
  const onSubmit = (e) => {
    ...
    if (leaveEmpError !== null) {
      ...
      setError(null);
      axios.post("http://localhost:3003/leave-requests", leaveRequest)
                  ...
                  .map((obj, index) => {
                    return (
                      <TableRow
                        ...
                        <TableCell align="center">
                        <Button 
                           onClick={() => viewLeaveDetails(obj.id)}
                           >
